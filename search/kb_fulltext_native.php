...
class kb_fulltext_native extends \sheer\knowledgebase\search\kb_base
{
	/**
	...
	*/
	...
	{
		return 'phpBB Knowledge Base Native Fulltext';
	}
	...
	*/
	public function keyword_search($type, $fields, $terms, $sort_by_sql, $sort_key, $sort_dir, $sort_days, $ex_fid_ary, $category_id, $author_ary, $author_name, $id_ary, $start, $per_page)
	{
		...
		if (!$total_results && $is_mysql)
		{
			...
			$sql_array_copy['SELECT'] = 'SQL_CALC_FOUND_ROWS p.article_id ';
...
//					$select = 'SQL_CALC_FOUND_ROWS ' . $select;
		...
		{
			// Count rows for the executed queries. Replace $select within $sql with SQL_CALC_FOUND_ROWS, and run it.
			$sql_calc = str_replace('SELECT ' . $select, 'SELECT DISTINCT SQL_CALC_FOUND_ROWS p.article_id', $sql);
	...
	* @param	int		$article_id		The id of the post which is modified/created
	* @param	string	&$message		New or updated article content
	* @param	string	&$subject		New or updated article title
...
//<--
	...
	/**
	* Removes entries from the wordmatch table for the specified article_ids
	*/
