...
class posting
{
	...
	*/
	...
	public function post_article()
	{
		...
		$action = append_sid("{$this->phpbb_root_path}knowledgebase/posting", 'id=' . $cat_id . '');
 		...
 		if ($mode == 'delete' || $delete)
		{
			...
			if (confirm_box(true))
			{
				...
				$root = append_sid("{$this->phpbb_root_path}knowledgebase/category",'id=' . $cat_id . '');
		...
		if ($mode == 'edit')
		{
			...
			$action = append_sid("{$this->phpbb_root_path}knowledgebase/posting",'mode=edit&amp;k=' . $art_id . '&amp;id=' . $cat_id . '');
		...
		if ($submit)
		{
			if ($article_title && $article_text && $article_description)
			{
				...
				if ($mode == 'edit')
				{
					...
					$redirect = append_sid("{$this->phpbb_root_path}knowledgebase/article",'k=' . $art_id . '');
				...
				else
				{
					...
					$redirect = append_sid("{$this->phpbb_root_path}knowledgebase/article",'k=' . $new . '');
					...
					else
					{
						...
						$this->helper->add_notification($sql_data, 'sheer.knowledgebase.notification.type.need_approval');
					}
		...
		if ($cancel)
		{
			redirect(append_sid("{$this->phpbb_root_path}knowledgebase/category",'id=' . $cat_id . ''));
		}
